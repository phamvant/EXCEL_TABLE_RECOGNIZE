...
from vietocr_structure.beam import Beam


def batch_translate_beam_search(img, model, beam_size=2, candidates=1, max_seq_length=256, sos_token=1, eos_token=2):
    # img: NxCxHxW
    model.eval()
    sents = []
    ...
    with torch.no_grad():
        ...
        for i in range(memories[1].size(1)):
            # memory = memories[:,i,:].repeat(1, beam_size, 1) # TxNxE
            memory = (memories[0][i, :].unsqueeze(0),
                      memories[1][:, i, :].unsqueeze(1))
            sent = beamsearch(memory, model, beam_size,
...
def translate_beam_search(img, model, beam_size=2, candidates=1, max_seq_length=256, sos_token=1, eos_token=2):
    # img: 1xCxHxW
    model.eval()
    ...
    with torch.no_grad():
        ...
        sent = beamsearch(memory, model, beam_size, candidates,
...
def beamsearch(memory, model, beam_size=4, candidates=1, max_seq_length=128, sos_token=1, eos_token=2):
    ...
    with torch.no_grad():
        hiddens = memory[0].repeat(beam_size, 1)  # TxNxE
        encoder_outputs = memory[1].repeat(1, beam_size, 1)
        memory = (hiddens, encoder_outputs)
...
def batch_translate(img, model, max_seq_length=256, sos_token=1, eos_token=2):
    "data: BxCXHxW"
    model.eval()
    device = img.device
    batch_translated_sentence = []
    with torch.no_grad():
        src = model.cnn(img)
        (hiddens, encoder_outputs) = model.transformer.forward_encoder(src)
        for i in range(encoder_outputs.size(1)):
            memory = (hiddens[i, :].unsqueeze(0),
    ...
    #            output = model.transformer(src, tgt_inp, tgt_key_padding_mask=None)
            ...
            batch_translated_sentence.append(list(translated_sentence[0]))
    batch_translated_sentence = np.asarray(batch_translated_sentence)
    return batch_translated_sentence
